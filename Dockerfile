...
ADD http://interactivebrokers.github.io/downloads/twsapi_macunix.973.06.zip .
RUN unzip twsapi_macunix.973.06.zip
...
COPY --from=twsapi-build \
    /tmp/twsapi-build/IBJts/source/pythonclient/dist/ibapi-9.73.6-py3-none-any.whl \
    /tmp/
RUN pip install /tmp/ibapi-9.73.6-py3-none-any.whl
